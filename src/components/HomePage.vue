    ...
    <el-row class="row">
        ...
        <el-card class="box-card">
          ...
          <el-row class="p-10">
            ...
            <el-col :span="24" :sm="18">
              <el-select
                @change="confChange(3, 0)"
                v-model="selectDataList[0].value"
                class="w-100"
                placeholder="Select"
                size="large"
              >
          ...
          <el-row
            v-for="(item, index) in audioConfigList"
            :key="index"
            class="p-10"
          >
            <el-col class="select-label" :span="24" :sm="6">{{
              item.label
            }}</el-col>
            <el-col :span="24" :sm="18">
              <el-slider
                @change="confChange(0, index)"
                v-model="item.value"
                :min="item.min"
                :max="item.max"
                class="w-100"
      ...
      <el-col class="col" :span="24" :lg="12">
        <el-card class="box-card">
          ...
          <el-row
            v-for="(item, index) in defaultPrompt"
            :key="index"
            class="p-10"
          >
            <el-col class="select-label" :span="24" :sm="6">{{
          ...
          <el-row
            v-for="(item, index) in openaiConfigList"
            :key="index"
            class="p-10"
          >
            <el-col class="select-label" :span="24" :sm="6">{{
  ...
  methods: {
    ...
    initConfig() {
      axios.get("/get_config").then((response) => {
        ...
        let openai_conf = data["oneapi"];
        this.defaultPrompt[0].value = openai_conf["default_prompt"];
        this.openaiConfigList[0].value = openai_conf["max_prompt_length"];
        this.openaiConfigList[1].value = openai_conf["ai_params"]["max_tokens"];
        this.openaiConfigList[2].value =
          openai_conf["ai_params"]["temperature"];
      });
    },
